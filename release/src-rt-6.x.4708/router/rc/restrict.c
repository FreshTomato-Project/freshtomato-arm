...
void ipt_restrictions(void)
{
	...
	for (nrule = 0; nrule < MAX_NRULES; ++nrule) {
		...
		/* 
		 * Check if bit 8 (value 8) is set.
		 * If set, user selected "All except..." (Whitelist). Matches RETURN.
		 * If not set, user selected "The following..." (Blocklist). Matches DROP/REJECT.
		 */
